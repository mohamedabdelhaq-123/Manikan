...
const plans = [
  {
    ...
    price: 'Free',
    period: null,
    description: 'Try SmartFit for up to 30 days. Full features, no card needed.',
...
const faqs = [
  ...
  },
  {
    ...
    a: 'SmartFit works with Shopify, WooCommerce, Salla, Zid, Magento, and any custom storefront via our JavaScript SDK.',
  },
  {
  ...
  return (
    <div className="pt-24 min-h-screen">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="text-center mb-12">
          ...
          <div className="flex items-center justify-center gap-3 mt-6">
            ...
            <button
              ...
              className={`relative w-12 h-6 rounded-full transition-colors ${billing === 'annual' ? 'bg-sage-500' : 'bg-gray-200'}`}
            >
            ...
            <span className={`text-sm ${billing === 'annual' ? 'text-gray-900 font-medium' : 'text-gray-400'}`}>
              Annual
              <Badge color="sage" className="ml-2 align-middle">Save 20%</Badge>
            </span>
          </div>
        </div>
            ...
            return (
              <div
                ...
                className={`relative rounded-2xl p-7 flex flex-col transition-all duration-200 ${
                  isHighlight
                    ? 'bg-sage-500 text-white shadow-lift md:scale-[1.03]'
                    : 'bg-white border border-warm-border shadow-soft hover:shadow-card'
                }`}
              >
                {isHighlight && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="bg-sand-500 text-white text-xs font-semibold px-3 py-1 rounded-full shadow-sm">
                      Most Popular
                    </span>
                  </div>
                )}
                ...
                <div className="mb-6">
                  <p className={`text-sm font-semibold mb-1.5 ${isHighlight ? 'text-sage-100' : 'text-sage-500'}`}>{plan.name}</p>
                  <div className="flex items-end gap-1.5 mb-2">
                    {plan.price !== 'Free' && (
                      <span className={`text-sm ${isHighlight ? 'text-sage-200' : 'text-gray-400'}`}>EGP</span>
                    )}
                    <span className={`text-4xl font-display ${isHighlight ? 'text-white' : 'text-gray-900'}`}>{displayPrice}</span>
                    {plan.period && (
                      <span className={`text-sm mb-1 ${isHighlight ? 'text-sage-200' : 'text-gray-400'}`}>/ {plan.period}</span>
                    )}
                  </div>
                  <p className={`text-sm leading-relaxed ${isHighlight ? 'text-sage-100' : 'text-gray-500'}`}>{plan.description}</p>
                </div>

                <ul className="space-y-2.5 mb-7 flex-1">
                  {plan.features.map((feat, j) => (
                    <li key={j} className="flex items-start gap-2.5">
                      <Check size={14} className={`mt-0.5 shrink-0 ${isHighlight ? 'text-sage-200' : 'text-sage-500'}`} />
                      <span className={`text-sm ${isHighlight ? 'text-sage-50' : 'text-gray-600'}`}>{feat}</span>
                    </li>
                  ))}
                </ul>
        ...
        {/* Pay per use */}
        <div className="bg-white rounded-2xl border border-warm-border shadow-soft p-7 mb-16">
          <div className="flex items-center gap-3 mb-6">
            <h2 className="text-xl text-gray-900">Pay-per-use model</h2>
            <Badge color="sand">Flexible</Badge>
          </div>
          <p className="text-sm text-gray-500 mb-6">
            Not ready for a monthly plan? Use SmartFit on a per-request basis — perfect for low-volume stores or seasonal campaigns.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {payPerUse.map((item, i) => (
              <div key={i} className="bg-warm-bg rounded-xl border border-warm-border p-4">
        ...
        {/* Comparison callout */}
        <div className="bg-sage-50 border border-sage-100 rounded-2xl p-7 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-16">
          <div>
            <h3 className="font-semibold text-sage-800 mb-1">Need to compare plans in detail?</h3>
            <p className="text-sm text-sage-600">See the full feature matrix or book a 20-minute call with our team.</p>
          </div>
        ...
        <div className="max-w-2xl mx-auto space-y-3">
          {faqs.map((faq, i) => (
            <div key={i} className="bg-white rounded-xl border border-warm-border shadow-soft overflow-hidden">
              <button
                onClick={() => setOpenFaq(openFaq === i ? null : i)}
                className="w-full flex items-center justify-between px-5 py-4 text-left"
              >
                <span className="text-sm font-medium text-gray-900 flex items-center gap-2">
                  <HelpCircle size={14} className="text-sage-400 shrink-0" />
                  {faq.q}
                </span>
              ...
              {openFaq === i && (
                <div className="px-5 pb-4 animate-fade-in">
                  <p className="text-sm text-gray-500 leading-relaxed pl-[22px]">{faq.a}</p>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="mt-16 text-center">
          <div className="bg-white rounded-2xl border border-warm-border shadow-card p-10 max-w-xl mx-auto">
            <Zap size={28} className="text-sage-500 mx-auto mb-4" />
            <h2 className="text-2xl text-gray-900 mb-2">Get started today.</h2>
